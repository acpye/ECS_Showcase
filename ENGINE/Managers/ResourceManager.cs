...
namespace OpenGL.ENGINE.Managers
{
    static class ResourceManager
    {
        ...
        {
            foreach (KeyValuePair<string, Geometry> geometry in geometryDictionary)
            {
                geometry.Value.RemoveGeometry();
            }
            geometryDictionary.Clear();
            foreach (KeyValuePair<string, int> texture in textureDictionary)
            {
                GL.DeleteTexture(texture.Value);
            }
            textureDictionary.Clear();
            foreach (KeyValuePair<string, int> audioBuffer in audioBufferDictionary)
            {
                AL.DeleteBuffer(audioBuffer.Value);
            }
        ...
        public static int LoadTexture(string filename)
        {
            ...
            if (texture == 0)
            {
                ...
                if (bmp == null)
                {
                    throw new FileNotFoundException($"Texture not found at '{Path.GetFullPath(filename)}'");
                }
